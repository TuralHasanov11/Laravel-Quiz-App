...
<div>
    <div class="md:grid md:gap-6">
      ...
      <div class="mt-5 md:mt-0 md:col-span-2">
        <form action="{{route('admin.quizzes.questions.update', ['quiz'=>$quiz, 'question'=>$question])}}" method="POST" enctype="multipart/form-data">
          <div class="shadow sm:rounded-md sm:overflow-hidden">
            <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
              ...
              <div class="grid gap-6">
                ...
                <div class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                    ...
                    @if ($question->image)
                        <img src="{{asset($question->image)}}" class="h-24 w-28 mx-auto" alt="{{$question->question}}">
                    @endif
                ...
                <div class="grid grid-cols-6 gap-6">
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <input type="text" name="answers[1]" id="answer1" value="@if (isset($question->answers[0])){{$question->answers[0]->answer}}@endif"
                            class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
    
                    <div class="col-span-6 sm:col-span-3">
                        <label for="answer2" class="block text-sm font-medium text-gray-700">2. Cavab</label>
                        <input type="text" name="answers[2]" id="answer2" value="@if (isset($question->answers[1])){{$question->answers[1]->answer}}@endif"
                            class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
    
                    <div class="col-span-6 sm:col-span-3">
                        <label for="answer3" class="block text-sm font-medium text-gray-700">3. Cavab</label>
                        <input type="text" name="answers[3]" id="answer3" value="@if (isset($question->answers[2])){{$question->answers[2]->answer}}@endif"
                            class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>

                    <div class="col-span-6 sm:col-span-3">
                        <label for="answer4" class="block text-sm font-medium text-gray-700">4. Cavab</label>
                        <input type="text" name="answers[4]" id="answer4" value="@if (isset($question->answers[3])){{$question->answers[3]->answer}}@endif"
                            class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
            ...
            <div class="px-4 py-3 bg-gray-50 text-left sm:px-6">
              <button onclick="event.preventDefault(); document.getElementById('delete-question-form').submit();"
        ...
        <form id="delete-question-form" action="{{route('admin.quizzes.questions.destroy', ['quiz'=>$quiz, 'question'=>$question])}}" method="post">
          @method('DELETE')
          @csrf
        </form>
      </div>
