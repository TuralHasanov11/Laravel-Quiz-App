<x-app-layout>
    <x-slot name="header">
        {{$quiz->title}} quizinə sual əlavə et
    </x-slot>
...
<div>
    <div class="md:grid md:gap-6">
      ...
      <div class="mt-5 md:mt-0 md:col-span-2">
        <form action="{{route('admin.quizzes.questions.store', ['quiz'=>$quiz])}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <select id="correct_answer" name="correct_answer" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            ...
                            <option value="1" @if (old('correct_answer')=='1')
                                    selected
                                @endif>1. Cavab
                            </option>
                            <option value="2" @if (old('correct_answer')=='2')
                                    selected
                                @endif>2. Cavab
                            </option>
                            <option value="3" @if (old('correct_answer')=='3')
                                    selected
                                @endif>3. Cavab
                            </option>
                            <option value="4" @if (old('correct_answer')=='4') 
                                    selected 
                                @endif>4. Cavab
                            </option>
                        </select>
